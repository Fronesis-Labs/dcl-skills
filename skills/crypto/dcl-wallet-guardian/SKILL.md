...
description: Validates wallet-affecting actions (approvals, transfers, signing requests) against policy before an agent executes them on behalf of a user. Use this skill whenever the user's request touches this specific check, even if not explicitly named.
---
...
Validates wallet-affecting actions (approvals, transfers, signing requests) against policy before an agent executes them on behalf of a user.
