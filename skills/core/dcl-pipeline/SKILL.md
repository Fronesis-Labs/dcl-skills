---
name: dcl-pipeline
description: Meta-skill that orchestrates the other core DCL skills as a single compliance pipeline (policy -> PII -> injection -> secrets -> drift). Use this skill whenever the user's request touches this specific check, even if not explicitly named.
---
...
Meta-skill that orchestrates the other core DCL skills as a single compliance pipeline (policy -> PII -> injection -> secrets -> drift).
